/**
 * @file server.hpp
 * @brief Заголовочный файл для HTTP/1.1 сервера с использованием epoll.
 *
 ...
 * Поддерживает TLS через OpenSSL 3.0+.
 *
...
/**
 * @brief Класс HTTP/1.1 сервера с использованием epoll.
 *
 ...
 * Использует современный API OpenSSL 3.0+.
 */
class Http1Server {
public:
    /**
     ...
     *
     * Освобождает ресурсы: SSL-контекст, SSL-соединения, epoll-дескриптор.
     */
    ~Http1Server();
...
private:
    // 👇 Структура для отслеживания незавершённых отправок
    struct PendingSend {
    ...
    void handle_new_connection() noexcept;
       ...
       /**
     * @brief Обрабатывает события ввода-вывода для указанного дескриптора.
     *
     * Обрабатывает данные от клиента или бэкенда, передавая их через forward_data.
     * Также проверяет таймауты и закрывает неактивные соединения.
     *
     * @param fd Дескриптор сокета, для которого произошло событие.
     * @param events_mask Маска событий (EPOLLIN, EPOLLOUT и т.д.).
     * @throws Никаких исключений — используется noexcept.
     */
    void handle_io_events(int fd, uint32_t events_mask) noexcept;
    ...
    /**
     * @brief Регистрирует дескриптор в epoll.
     * @param fd Дескриптор сокета.
     * @param events События (EPOLLIN, EPOLLOUT и т.д.).
     * @return true при успехе, false при ошибке.
     */
    [[nodiscard]] bool add_epoll_event(int fd, uint32_t events) noexcept;
     ...
     * @return true при успехе, false при ошибке.
     */
    [[nodiscard]] bool remove_epoll_event(int fd) noexcept;
};