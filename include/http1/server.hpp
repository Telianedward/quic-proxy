...
private:
    ...
    std::atomic<int> active_connections_{0}; ///< Текущее количество активных соединений
    static constexpr int MAX_CONNECTIONS = 100; ///< Максимальное количество одновременных соединений
     ...
     * @param info Информация о соединении.
     */
    void handle_client_data(int client_fd, ConnectionInfo& info) noexcept;

    /**
     * @brief Обрабатывает данные от бэкенда.
     * @param client_fd Дескриптор сокета клиента.
     * @param info Информация о соединении.
     */
    void handle_backend_data(int client_fd, ConnectionInfo& info) noexcept;
     ...
     * @param info Информация о соединении.
     */
    void close_connection(int client_fd, ConnectionInfo& info) noexcept;

    /**
     * @brief Выполняет TLS handshake для соединения.
     * @param client_fd Дескриптор сокета клиента.
     * @param info Информация о соединении.
     * @return true, если handshake успешно завершён, false в случае ошибки.
     */
    bool perform_tls_handshake(int client_fd, ConnectionInfo& info) noexcept;

    /**
     * @brief Обрабатывает незавершённые отправки для заданного сокета.
     * @param fd Дескриптор сокета.
     * @return true, если соединение активно, false в случае ошибки.
     */
    bool process_pending_sends(int fd) noexcept;

    /**
     * @brief Пытается отправить данные на целевой сокет.
     * @param fd Целевой сокет.
     * @param data Указатель на данные.
     * @param len Длина данных.
     * @param ssl Указатель на SSL-объект (nullptr, если нет TLS).
     * @return Количество отправленных байт, или -1 в случае ошибки.
     */
    ssize_t try_send(int fd, const char* data, size_t len, SSL* ssl) noexcept;
    // В private секции класса Http1Server
std::unordered_map<int, size_t> content_length_; // Ключ — client_fd, значение — ожидаемая длина тела
std::unordered_map<int, bool> headers_received_; // Ключ — client_fd, значение — true, если заголовки получены
};
