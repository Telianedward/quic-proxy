...
int main() {
    ...
    //   Http2Server http2_server(http2_port, backend_ip, backend_http1_port); // 👈 Передаём backend_ip и backend_http1_port
        ...
        // quic_thread.join();
        // tcp_thread.join();
        // 👇 ДОБАВЛЯЕМ ПРОВЕРКУ И ПРИНУДИТЕЛЬНОЕ ЗАВЕРШЕНИЕ
        // Убедимся, что серверы действительно остановились
       if (http1_server.is_running()) {
            LOG_WARN("⚠️ HTTP/1.1 сервер не остановился автоматически. Принудительное завершение...");
            http1_server.stop();
            // НЕ ЖДЁМ — join() дождётся завершения потока сам
        }
