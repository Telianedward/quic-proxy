...
bool Http1Server::forward_data(int from_fd, int to_fd, SSL *ssl) noexcept
{
    ...
    // 🟢 Обработка HTTP-заголовков, если это ответ от бэкенда
    if (from_fd == connections_[to_fd].backend_fd) // Это ответ от бэкенда
    {
        // Ищем конец заголовков (двойная новая строка)
        const char* headers_end = strstr(buffer, "\r\n\r\n");
        if (headers_end && !headers_received_[to_fd])
        {
            headers_received_[to_fd] = true;
            ...
            if (content_length_header)
            {
                ...
                while (*content_length_header == ' ' || *content_length_header == '\t') content_length_header++;
                char* endptr;
                long cl = strtol(content_length_header, &endptr, 10);
                if (endptr != content_length_header && cl >= 0)
                {
                    content_length_[to_fd] = static_cast<size_t>(cl);
                    LOG_INFO("📌 Content-Length найден: {}", content_length_[to_fd]);
                }
            }
        }

        // Если есть Content-Length, проверяем, сколько данных уже отправлено
        if (content_length_.find(to_fd) != content_length_.end())
        {
            size_t total_sent = 0;
            // Суммируем все отправленные данные
            for (const auto& conn : connections_)
            {
                if (conn.first == to_fd)
                {
                    total_sent = 0; // Здесь нужно отслеживать общее количество отправленных байт
                    break;
                }
            }
            // Это упрощённая логика — вам нужно будет доработать её
            if (total_sent >= content_length_[to_fd])
            {
                LOG_INFO("✅ Все данные отправлены. Закрываем соединение для клиента {}", to_fd);
                close_connection(to_fd, connections_[to_fd]);
                return false;
            }
        }
    }
