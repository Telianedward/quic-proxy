...
bool Http1Server::forward_data(int from_fd, int to_fd) noexcept
{
    ...
    *           - 0: флаги — без специальных опций.
    ...
    if (bytes_read > 0)
    {
        ...
        // 🟢 ЛОГИРУЕМ СОДЕРЖИМОЕ ЗАПРОСА (ТОЛЬКО ЕСЛИ ЭТО КЛИЕНТСКИЙ СОКЕТ)
        if (from_fd == client_fd)
        { // ← ВАЖНО: проверяем, что источник — клиент
            ...
            LOG_DEBUG("📋 Получен HTTP-запрос от клиента:\n{}", request.substr(0, 512)); // первые 512 байт
        }
