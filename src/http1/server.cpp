...
bool Http1Server::forward_data(int from_fd, int to_fd, SSL *ssl) noexcept
{
    ...
    if (bytes_read > 0)
    {
        ...
        while (total_sent < bytes_read)
        {
            ...
            // ❌ Но это неверно! Если `from_fd` — это бэкенд, а `to_fd` — это клиент, то нам НУЖНО использовать SSL!
            ...
            // 🟢 ВРЕМЕННОЕ ИСПРАВЛЕНИЕ: Для тестирования, пока мы не меняем архитектуру, будем считать, что:
            // Если `use_ssl` == true, то мы читаем через SSL (это клиент), и отправляем на бэкенд — без SSL.
            // Если `use_ssl` == false, то мы читаем с бэкенда и отправляем клиенту — и тут нам нужен SSL!
            // ❗ Это НЕВЕРНО! Нам нужно знать, кто получатель.

            // 🟡 ПРАВИЛЬНОЕ ИСПРАВЛЕНИЕ (для текущей структуры):
            // Мы не можем использовать `ssl` для отправки, потому что `ssl` связан с `from_fd`, а не с `to_fd`.
            // 🚫 Поэтому, чтобы отправить данные на клиент через TLS, нам нужно получить `SSL*` по `to_fd`.
     ...
     // 🟡 ОБРАБОТКА УСПЕШНОЙ ОТПРАВКИ (bytes_sent > 0)
            ...
            // 🟠 ЕСЛИ МЫ ЕЩЁ НЕ ОТПРАВИЛИ ВСЕ ДАННЫЕ — ВЫХОДИМ ИЗ ФУНКЦИИ, ЧТОБЫ SELECT() МОГ РАБОТАТЬ
            if (total_sent < bytes_read)
            {
                LOG_DEBUG("⏳ Не все данные отправлены ({} из {}). Ждём следующего цикла select().", total_sent, bytes_read);
                return true; // Возвращаем true, чтобы основной цикл мог продолжить работу
            }
        }
