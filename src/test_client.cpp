...
int main()
{
    try
    {
        ...
        const int backend_port = 8586;              // Порт, на который отправляем (должен совпадать с тем, куда отправляет клиент)
        ...
        if (connect(sock, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0)
        {
            ...
            {
                LOG_ERROR("❌ Не удалось подключиться к серверу: {}", strerror(errno));
                close(sock);
                return EXIT_FAILURE;
            }
        }

        LOG_SUCCESS("✅ Подключился к серверу {}:{}", backend_ip, backend_port);
        ...
        if (received_bytes > 0)
        {
            ...
            LOG_SUCCESS("✅ Получено сообщение от сервера: \"{}\"", buffer);
        }
        else if (received_bytes == 0)
        {
            // Соединение закрыто сервером
            LOG_INFO("ℹ️ Сервер закрыл соединение");
        }
        else
        {
            LOG_ERROR("❌ Ошибка получения сообщения: {}", strerror(errno));
        }
